...
let connect ~var ~server fn =
	let open Lwt_unix in
	let s = make_socket () in
	let my_address = server_address (Unix.getpid ()) in
	Log.debug var (fun m->m "connecting to server %s from %s" server (my_address));
	let%lwt () = Lwt_unix.bind s (ADDR_UNIX my_address) in
	let state = {
		next_id = ref 0;
		socket = s;
	...
	let loop = listen_loop ~var state.socket (fun bytes _sender ->
